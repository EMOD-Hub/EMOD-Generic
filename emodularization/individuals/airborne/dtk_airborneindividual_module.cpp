...
Kernel::IndividualHumanAirborne * person = nullptr;
...
std::map< int, Kernel::IndividualHumanAirborne * > population;
...
static void setConfigJson()
{
    ...
    configStubJson = Configuration::Load("ai.json");
#ifndef WIN32
...
static Kernel::IndividualHumanAirborne* initInd( int sex, float age, float mcw )
{
    person = Kernel::IndividualHumanAirborne::CreateHuman( &node, (*individualHumanSuidGenerator)(), mcw, age, sex );
    if( configStubJson == nullptr )
    {
        //configStubJson = Configuration::Load("gi.json");
        setConfigJson();

        std::cout << "configStubJson initialized from ai.json." << std::endl;
...
create(PyObject* self, PyObject* args)
{
    ...
    catch( const std::exception& exc )
    {
        ...
        PyErr_SetString( PyExc_RuntimeError, "ai.json file not found probably." );
        return NULL;
    }
}
...
static PyMethodDef GenericIndividualMethods[] =
{
     ...
     {"should_infect", shouldInfect<Kernel::IndividualHumanAirborne>, METH_VARARGS, "Ask intrahost module if transmission should occur."},
     {"get_individual_for_iv", getIndividual<Kernel::IndividualHumanAirborne>, METH_VARARGS, "Get PyCapsule pointer to pass to intervention."},
...
PyMODINIT_FUNC
PyInit_dtk_airborne_intrahost(void)
{
