...
transmission from one IP group to another group. This SFT tests more IP values(10) and larger HINT matrix range. 
Please see test comment in Environmental\SFTs\Multi_Route_HINT\Single_Property_Between_Groups for more details.

This test validates 7 things:

1. Simulation configuration preconditions for the test are met.
2. For each group except the first group(for example, Geographic:BB):
    2.1 Total contagion for contact route should be Base_Infectivity_Constant * number of infected in group Geographic:AA * 
        multiplier in HINT matrix / total population
    2.2 Total contagion for environmental route should be Base_Infectivity_Constant * number of infected in group Geographic:AA * 
...
5. For the first group(seed group) Geographic:AA, contagions should be zero for all time steps.
    ...
    with open(report_name, "w") as sft_report_file :
        ...
        if int(param_obj[ConfigKeys.Enable_Heterogeneous_Intranode_Transmission]) != 1:
            ...
            if not all(x == 1 for x in campaign_obj[CampaignKeys.Demographic_Coverage]):
                sft_report_file.write("WARNING: {} should be 1, please check campaign.json."
            ...
            seed_group = property_values[0]
            expected_property_restrictions.append(["{0}:{1}".format(
                    property_obj[DemographicsKeys.PropertyKeys.Property], seed_group)])
        ...
        if success:
            ...
            for susceptible_group in property_values[1:]: # skip the first group
                ...
                with open("DEBUG_binomial_test_{}.txt".format(susceptible_group), 'w') as group_file:
                    for t in range(1, duration - 1):
                        infectivity_seed = base_infectivity * infected_seed.iloc[t-1, 0]
                        ...
                        if math.fabs(calculated_contagion_e - actual_contagion_e) > 5e-2 * calculated_contagion_e:
                            ...
                            sft_report_file.write("    BAD: at time step {0}, for group {1} route {2}, the contagion is {3}, "
                                          "expected {4}.\n".format(t, susceptible_group, routes[0],
                                                                   actual_contagion_e,
                        ...
                        if math.fabs(calculated_contagion_c - actual_contagion_c) > 5e-2 * calculated_contagion_c:
                            result_2 = success = False
                            sft_report_file.write("    BAD: at time step {0}, for group {1} route {2}, the contagion is {3}, "
                                          "expected {4}.\n".format(t, susceptible_group, routes[1],
                                                                   actual_contagion_c,
                        ...
                        susceptible_population_c = population_sus.iloc[t, 0] - infected_sus.iloc[t-1, 0]
                        expected_new_infection_c = susceptible_population_c * calculated_prob_c

                        # Superinfection is not supported, so individuals may not be infected from both the
                        # environmental and the contact route. As per the requirement (https://github.com/ ... specs/
                        # specs/environmental_sim-spec.rst#infections), contact transmission happens first.
                        # t+1 is because the new infection isn't reported until the next timestep.
                        ...
                        # run the same test for both routes in this for loop
                ...
                for contagion_list, new_infection_sus, expected_new_infection_list, route in \
                    ...
                    if total_expected_new_infections > 1000:
                        ...
                        sft_report_file.write("WARNING: the new infection size for route {0} and group {1} is too " 
                                              ...
                                              "got {3}.\n".format(route, susceptible_group,
                    ...
                    if tolerance > 0:
                        if math.fabs(total_new_infections - total_expected_new_infections) > \
                            ...
                            sft_report_file.write(message.format("BAD", route, susceptible_group, total_new_infections,
                                                                 total_expected_new_infections))
                        else:
                            sft_report_file.write(message.format("GOOD", route, susceptible_group, total_new_infections,
                                                                 total_expected_new_infections))
                    ...
                    sft.plot_data(np.array(contagion_list)[:, 0], np.array(contagion_list)[:, 1],
                                      ...
                                      xlabel='day', ylabel='contagion', category="contagion_{0}_{1}"
                                                                                 "".format(susceptible_group, route),
            ...
            sft_report_file.write("Test 5: make sure the first group {} has no contagion.\n".format(
                expected_property_restrictions))
            result_5 = True
            ...
            sft_report_file.write("Test 6: testing {0} and {1} in {2} and data collected form {3} for every time step.\n".format(
                InsetKeys.ChannelsKeys.New_Infections_By_Route_ENVIRONMENT,
                InsetKeys.ChannelsKeys.New_Infections_By_Route_CONTACT, insetchart_name, property_report_name))
            result_6 = hint_support.compare_new_infections_channels(inset_channels[:2], property_df, duration,
            ...
            sft_report_file.write("Test 7: testing {0} and {1} in {2} and data collected form {3} for every time step.\n".format(
                InsetKeys.ChannelsKeys.Environmental_Contagion_Population,
                InsetKeys.ChannelsKeys.Contact_Contagion_Population, insetchart_name, property_report_name))
            result_7 = hint_support.compare_contagion_channels(inset_channels[2:], channels[3:5], property_df, duration,
                                                               inset_chart_obj, sft_report_file, insetchart_name,
                                                               property_report_name)
            if not result_7:
                success = False
            sft_report_file.write("Test 7: result is: {}.\n".format(result_7))
