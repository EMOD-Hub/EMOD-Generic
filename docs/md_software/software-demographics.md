...
population in each **node** in the simulation. For example, the longitude and latitude, the
...
In addition, demographics files are useful for creating heterogeneity in a population. You can define
...
see [model-properties](../md_model/model-properties.md).
...
information, see [model-care-cascade](../md_model/model-care-cascade.md).

EMOD assumes homogeneous mixing and disease transmission for the generic simulation type. Using
the individual properties in the demographics file, you can also use the HINT feature to add
heterogeneous transmission to your generic model. You cannot manually configure heterogeneous
transmission using HINT with other simulation types because the heterogeneity in transmission
for specific diseases and disease classes is configured by transmission parameters specific to each
disease model. For more information, see [model-hint](../md_model/model-hint.md).
