...
the Eradication.exe. A custom reporter is an **EMODule** that you plug in to EMOD. Custom reporters
...
![images/dev/Reporters.png](images/dev/Reporters.png)
...
1. Define the location of the **dynamic link library (DLL)** in a JSON-formatted file (emodules_map.json). #.  Point to the location using the `--dll-path` command-line option when invoking Eradication.exe. #.  Place the DLL in the **working directory**.

## JSON file

First, Eradication.exe looks for an emodules_map.json file in the working directory. This map file lists
...
For more information on EMOD command-line options, see [software-simulation-cli](software-simulation-cli.md).

## Working directory

Finally, if neither the the map file or the command-line option are provided, Eradication.exe looks for the
