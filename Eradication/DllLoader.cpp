...
)
{
    ...
    std::list< emodulewstr > disease_dll_dirs;
    ...
    if( disease_dll_dirs.size() == 0 )
    {
        ...
        dllDir = GetFullDllPath(emodulewstr(DISEASE_PLUGINS));
                                ...
                                const char* dllName)
{
    ...
    for (auto& dllDirStar : report_dll_dirs)
    {
        ...
        do
        {
            ...
            newSimDlHandle = dlopen( fullDllPath.c_str(), RTLD_LAZY|RTLD_LOCAL );
            if( newSimDlHandle )
            {
