...
namespace Kernel
{
    ...
    public:
        ...
        virtual uint64_t CreateBits( const std::vector<std::string>& rGenomeMarkerNames ) const override;
        ...
        virtual std::vector<std::pair<std::string, uint64_t>> CreatePossibleCombinations() const override;

        // Return the number of markers
        virtual uint64_t Size() const override;

        // Return the bits for the given name.  This is an integer with the bit set for this marker.
        virtual uint64_t GetBits( const std::string& rName ) const override;
        ...
        static uint64_t FromOutcrossing( RANDOMBASE* pRNG, uint64_t id1, uint64_t id2 );

    protected:
        const std::pair<std::string, uint64_t>* Get( const std::string& rName ) const;

        std::map<std::string,std::pair<std::string,uint64_t>> m_MarkerMap;  // this maps lowercase name to input name/bits
